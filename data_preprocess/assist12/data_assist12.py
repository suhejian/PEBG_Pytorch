...
class DataProcess():
    ...
    def pro_skill_graph(self):
        ...
        for pro_id in tqdm(pro_id_dict.keys()):   
            ...
            assert len(tmp_df) > 0, pro_id
            tmp_df_0 = tmp_df.iloc[0]

            # pro_feature: [ms_of_response, problem_type, mean_correct_num]
            ...
            tmp_skills = [ str(tmp_df_0['skill_id']) ]
            for s in tmp_skills:
                assert s in skill_id_dict, (s, pro_id, tmp_df_0['skill_id'])
...
if __name__ == '__main__':
    # 存放assist12数据集的目录, 里面包括干净csv文件和hashmap
    data_folder = '/data/hejiansu/16-service/sequence_data/assist12'
    min_inter_num = 3
    file_name= 'assist12.csv'
