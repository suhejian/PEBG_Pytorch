...
class DataProcess():
    ...
    # def func(self, df, pro_id, pro_feat, pro_skill_adj, pro_id_dict, skill_id_dict):
    #     # import ipdb; ipdb.set_trace()
    #     tmp_df = df[df['problem_id']==pro_id]
    #     assert len(tmp_df) > 0
    #     tmp_df_0 = tmp_df.iloc[0]

    #     # pro_feature: [ms_of_response, answer_type, mean_correct_num]
    #     tmp_pro_feat = [0.] * 2
    #     tmp_pro_feat[0] = tmp_df['elapsed_time'].abs().mean()
    #     tmp_pro_feat[-1] = tmp_df['correct'].mean()
    ...
    def pro_skill_graph(self):
        ...
        # p = Pool(processes=10)
        # f = lambda pro_id: func(df, pro_id, pro_feat, pro_skill_adj, pro_id_dict, skill_id_dict)
        # p.map(f, pro_id_dict.keys())
        # p.close()
        # p.join()
...
if __name__ == '__main__':
    # 存放ednet数据集的目录, 里面包括干净csv文件和hashmap
    data_folder = '/data/hejiansu/16-service/sequence_data/ednet'
    min_inter_num = 3
    file_name= 'ednet.csv'
